    ...
    app.post("/create-intent", {
        ...
        handler: async (request, reply) => {
            ...
            try {
                ...
                const stripePrice = await stripe_1.stripe.prices.retrieve(priceId);
                if (!stripePrice.active) {
                    return reply.code(400).send({ message: "Preço não está ativo." });
                }
                // Buscar informações do produto no Stripe
                const stripeProduct = await stripe_1.stripe.products.retrieve(stripePrice.product);
                ...
                // Método 2: Mapeamento baseado no priceId (fallback)
                    ...
                    // Mapeamento de exemplo - ajuste conforme seus preços
                    const priceMapping = {
                ...
                const transaction = await prisma_1.prisma.creditTransaction.create({
                    data: {
                        type: "PURCHASE",
                        status: "PENDING",
                ...
                await prisma_1.prisma.creditTransaction.update({
    ...
    app.post("/confirm", {
        ...
        handler: async (request, reply) => {
            ...
            try {
                ...
                const transaction = await prisma_1.prisma.creditTransaction.findUnique({
                ...
                if (paymentIntent.status === "succeeded") {
                    ...
                    // Pagamento falhou
                    const updatedTransaction = await prisma_1.prisma.creditTransaction.update({
                        where: { id: transaction.id },
                        data: {
                            status: "FAILED",
                            metadata: {
        ...
        handler: async (request, reply) => {
            ...
            try {
                ...
                if (productId) {
                    ...
                    const product = await stripe_1.stripe.products.retrieve(productId);
                    products = [product];
                }
                else {
                    // Listar todos os produtos ativos
                    const productsList = await stripe_1.stripe.products.list({
                        active: true,
                        limit: 100,
                    });
    ...
    app.get("/:userId/history", {
        ...
        handler: async (request) => {
            ...
            const [total, transactions] = await Promise.all([
                prisma_1.prisma.creditTransaction.count({ where }),
    ...
    app.post("/webhook", {
        ...
        handler: async (request, reply) => {
            ...
            try {
                ...
                event = stripe_1.stripe.webhooks.constructEvent(request.body, sig, stripe_1.STRIPE_WEBHOOK_SECRET);
            }
            catch (err) {
